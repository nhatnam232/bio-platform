    ...
    <div className="relative min-h-screen text-white flex items-center justify-center p-6">
      ...
      <Frame accent="#ffffff" className="relative z-10 w-full max-w-sm p-8 fade-up rounded-xl">
        <form onSubmit={handleLogin} className="flex flex-col gap-4">
          <div>
            <div className="text-[11px] font-semibold uppercase tracking-[0.25em] text-zinc-500">welcome back</div>
            <h1 className="text-3xl font-bold mt-1">Đăng nhập</h1>
          </div>
          ...
          <button type="submit" disabled={loading} className="w-full py-2.5 bg-white text-black font-semibold rounded-md hover:bg-zinc-200 disabled:opacity-50 transition">{loading ? '...' : 'Đăng nhập'}</button>
          <p className="text-sm text-zinc-500 text-center">Chưa có tài khoản? <Link to="/register" className="text-white font-medium hover:underline">Đăng ký</Link></p>
        </form>
      </Frame>
    </div>
  )
