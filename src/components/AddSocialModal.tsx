...
export default function AddSocialModal({
  ...
  onClose: () => void
  onAdd: (link: { label: string; url: string; icon: string; platform: string; mode: LinkMode }) => void
}) {
  ...
  const submit = () => {
    ...
    onAdd({ label: sel.label, url, icon: sel.icon, platform: sel.id, mode })
    onClose()
  }
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/85 backdrop-blur-sm fade-in p-4" onClick={onClose}>
      <div onClick={stop} className="relative w-full max-w-sm border border-white/15 bg-zinc-950 rounded-lg p-5 flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <h3 className="font-semibold">Add Social</h3>
        ...
        <div>
          ...
          <div className="grid grid-cols-7 gap-2">
            {SOCIALS.map((s) => {
              const active = sel.id === s.id ? 'border-white bg-white/15' : 'border-white/10 hover:border-white/30'
              return (
                <button key={s.id} title={s.label} onClick={() => setSel(s)} className={'aspect-square flex items-center justify-center text-lg rounded-md border transition ' + active}>{s.icon}</button>
              )
            })}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex border border-white/10 rounded-md overflow-hidden">
            <button onClick={() => setMode('text')} className={'flex-1 py-2 text-sm transition ' + (mode === 'text' ? 'bg-white text-black' : 'text-zinc-400 hover:text-white')}>Text</button>
            <button onClick={() => setMode('link')} className={'flex-1 py-2 text-sm transition ' + (mode === 'link' ? 'bg-white text-black' : 'text-zinc-400 hover:text-white')}>Link</button>
          </div>
        </div>
        ...
        <button onClick={submit} className="w-full py-2 bg-white text-black font-medium rounded-md hover:bg-zinc-200 transition">Submit</button>
      </div>
    </div>
  )
}
